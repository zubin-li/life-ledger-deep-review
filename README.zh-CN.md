...
- **两个 Google 账号：** 最多连接两个 Google 日历账号，并自行选择需要显示的日历。
- **只读每日日历：** 临时日程清晰呈现，重复日程可以默认收起；Life Ledger 不会修改 Google 日历。
- **更安静的今日页面：** 每日日历与每日复盘各占一半，不再重复放置每日灵活目标。
- **可迁移的历史：** 经过验证的 JSON 导出与恢复，方便更换设备或浏览器。
